...
CONTAINS
  ...
  !========================================================
    ...
    REAL(KIND=8), DIMENSION(N_dim) :: a_m, int_G_i
    ...
    !   T_par = ele%volume / MAXVAL(K_i)
    !   T_par = ele%volume / K_plus
    
!!$    beta_i = ( 1.d0 / 3.d0 ) + &
!!$             (0.5d0 * l_max * K_i)/(mod_am * ele%volume)
...
!!$       beta_i(i) = omega_i(i) + &
!!$                   0.5d0 * (T_par / ele%volume) * DOT_PRODUCT(a_m, int_G_i)
